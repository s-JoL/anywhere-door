...
12. **Timeline hygiene.** Keep regenerate/rewind/fork from leaking old-branch
    state.
13. **Two single-language deployments + UI redesign (zh/en).** Stand up the
    bilingual model (`product-design.md` §2.5, `architecture.md` §5.5) as **two
    separate deployments of one kernel**, locale fixed at build/deploy time (not a
    runtime switch): extract every inline string and language-facing prompt behind
    a build-time locale constant; native per-language UI/typography; a per-language
    story/seed content pool. Paired with the broader UI optimization in
    `ui-redesign-proposal.md` (information architecture, feed/play/library
    redesign, a shared design system with two native locale skins). The
    world-running engine stays one shared, language-agnostic kernel — identifiers
    only, no per-language runtime fork (charter §15.14). Sequencing in the proposal.
