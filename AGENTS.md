...
The axiom is the root of the *consistency* invariants. A few **execution-layer**
choices (local-first / BYO-key cost control, ambient-by-default, Consequence Mode)
are *co-determined* — pointed at independently by physical constraints (per-turn
cost, finite context) as much as by the axiom. That over-determination is a source
of confidence, not a weakening; the derivation is in `first-principles.md` §1.6.
...
fact snapshot is the source material, so grounding is **biased structural**.
Faithful is the default; **lawful distortion** (horror sanity effects, dream
logic, a world that hides a death) is a rules-level property of the world, not an
ad-hoc switch. The hub always holds the real truth underneath, so a *caught* slip
is recoverable, not a divergence.

Because narration is still model-generated, a **consistency guard** remains. It is
**conservative and best-effort**: it screens the cheap, high-value cases — prose
that names an entity absent from the snapshot, asserts a state the snapshot
contradicts, or gives a character knowledge outside its projection — and is *not*
a full semantic re-derivation. Subtle slips can pass, so the honest claim is
*biased grounding + a backstop*, not a proof of non-divergence; grounding comes
from generating-from-truth, never from the guard. Character voices are an
orthogonal layer — a partial perceiver may lie or err; such claims route to that
character's belief or a recorded lie, never silently into state. (Mechanism in
`architecture.md` §8.)
