...
const JSON_SHAPE = `{
  ...
  "rules": {
    ...
    "narrationRule": "叙述规则：如何把已提交事实快照转述成有生命的旁白；可包含 lawful distortion，但必须说明不发明快照外实体"
  },
  ...
  "presentation": {
    ...
    "entryAction": "Feed 按钮文案：8-16 字的第一步行动或开场发言，必须具体、可点击，不要写『推门进入』",
...
- rules.narrationRule 必须说明这个世界如何把「已提交事实快照」转述成旁白：默认忠实、有感官细节、不发明快照外新实体；如果是恐怖/梦境/失真世界，lawful distortion 写在这里。
- 冷开场 hook 必须是第二人称「你」，1-3 句，结尾悬在张力上或抛出一个行动邀请——这是 feed 里 1 秒钟就要抓住人的钩子。
- presentation.entryAction 必须是 feed 按钮上的第一步行动/开场发言，短、具体、诱人点击；绝不能写「推门进入」「进入世界」这种抽象入口。
...
export function parseGeneratedSeed(
  ...
  try {
    ...
    const rules: WorldRules = {
      ...
      narrationRule: str(rawRules.narrationRule) || DEFAULT_NARRATION_RULE,
    };
    ...
    const entryAction = str(rawPres.entryAction);
