...
function obs(charId: string, text: string, importance: number): Memory {
  return { id: `${charId}-${text}`, charId, kind: "observation", text, keywords: [], importance, createdAt: 1, lastAccessed: 1 };
}
...
describe("propagateGossip (co-location 传话)", () => {
  ...
  it("does not spread low-importance idle chatter", () => {
    ...
    expect(propagateGossip(present, recent)).toEqual([]);
  });
  ...
  it("does not re-tell a listener something they already heard (dedup)", () => {
    ...
    const recent = {
      ...
      b: [{ id: "b-old", charId: "b", kind: "hearsay" as const, text, keywords: [], importance: 5, createdAt: 1, lastAccessed: 1 }],
    };
    expect(propagateGossip(present, recent).some((m) => m.charId === "b")).toBe(false);
  });

  it("does not re-propagate hearsay as if it were first-hand", () => {
    const recent = { a: [{ id: "a-h", charId: "a", kind: "hearsay" as const, text: "听别人说的事", keywords: [], importance: 9, createdAt: 1, lastAccessed: 1 }], b: [] };
    expect(propagateGossip(present, recent)).toEqual([]);
  });

  it("needs at least two co-present characters", () => {
    const recent = { a: [obs("a", "阿岚：大事发生了！", 9)] };
    expect(propagateGossip([{ id: "a", name: "阿岚" }], recent)).toEqual([]);
  });
});
